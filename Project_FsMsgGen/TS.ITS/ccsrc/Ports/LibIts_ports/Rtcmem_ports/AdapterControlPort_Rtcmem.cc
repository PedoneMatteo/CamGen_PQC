...
namespace LibItsRtcmem__TestSystem {
  ...
  void AdapterControlPort::outgoing_send(const LibItsCommon__TypesAndValues::AcSecPrimitive &send_par) {
    ...
    RtcmemLayer *p = registration<RtcmemLayer>::get_instance().get_item(std::string("RTCMEM"));
    if (p != NULL) {
      ...
      loggers::get_instance().error("AdapterControlPort::outgoing_send: RTCMEM not registered");
    }
  }
