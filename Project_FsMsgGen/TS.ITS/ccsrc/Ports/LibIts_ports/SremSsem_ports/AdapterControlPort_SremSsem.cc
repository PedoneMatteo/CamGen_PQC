...
namespace LibItsSremSsem__TestSystem {
  ...
  void AdapterControlPort::outgoing_send(const LibItsCommon__TypesAndValues::AcSecPrimitive &send_par) {
    ...
    SremSsemLayer *p = registration<SremSsemLayer>::get_instance().get_item(std::string("SremSsem"));
    if (p != NULL) {
      ...
      loggers::get_instance().error("AdapterControlPort::outgoing_send: SremSsem not registered");
    }
  }
