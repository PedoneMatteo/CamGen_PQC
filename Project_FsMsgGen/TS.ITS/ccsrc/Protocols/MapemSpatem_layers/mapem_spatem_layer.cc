...
void mapem_spatem_layer::sendMsg(const LibItsMapemSpatem__TypesAndValues::MapemReq &p, params &params) {
...
void mapem_spatem_layer::sendMsg(const LibItsMapemSpatem__TypesAndValues::SpatemReq &p, params &params) {
  loggers::get_instance().log_msg(">>> mapem_spatem_layer::sendMsg: ", p);
...
void mapem_spatem_layer::receive_data(OCTETSTRING &data, params &params) {
  ...
  if (*(static_cast<const unsigned char *>(data) + 1) == 0x05) { // Check that received packet has MAPE message id
    ...
    LibItsMapemSpatem__TypesAndValues::MapemInd p;
    ...
    LibItsMapemSpatem__TypesAndValues::SpatemInd p;
...
int mapem_spatem_layer::enable_secured_mode(const std::string &p_certificate_id, const boolean p_enforce_security) {
  ...
  loggers::get_instance().log("mapem_spatem_layer::enable_secured_mode: Got GN layer %p", p);
...
int mapem_spatem_layer::disable_secured_mode() {
  ...
  loggers::get_instance().log("mapem_spatem_layer::disable_secured_mode: Got GN layer %p", p);
